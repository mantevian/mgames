...
fun setBlock(level: ServerLevel, pos: Vec3i, block: Block) {
    level.setBlock(BlockPos(pos), block.defaultBlockState(), 2)
}

fun getBlock(level: ServerLevel, pos: Vec3i): BlockState {
    return level.getBlockState(BlockPos(pos))
}
...
fun island(
        ...
        for (x in -radius..radius) {
            for (z in -radius..radius) {
                for (y in 0..height) {
                    ...
                    if (blockDown.isAir && blockHere.`is`(surfaceBlock)) {
    ...
    while (attempts < radius * radius && treesLeft > 0) {
        ...
        if (getBlock(server.overworld(), pos)
                .`is`(surfaceBlock)
            && placeFeature(treeFeature, BlockPos(pos.x, pos.y + 1, pos.z))
        ) {
            treesLeft--
            attempts = 0
            continue
...
fun getFeature(name: String): ConfiguredFeature<*, *>? {
    val registry = getRegistry(Registries.CONFIGURED_FEATURE)
    return registry.get(Identifier.parse("minecraft:$name")).getOrNull()?.value()
}
...
fun placeFeature(name: String, pos: BlockPos): Boolean {
    ...
    return feature.place(
        server.overworld(),
        server.overworld().chunkSource.generator,
